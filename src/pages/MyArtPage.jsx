...
const MyArtPage = () => {
  const { user } = useAuth();
  const { data, refetch } = useMyArt(user);
          ...
          <div className="mb-0 mt-8 flex justify-center">
            <div className="dropdown dropdown-end">
              <div
                tabIndex={0}
                role="button"
                className="flex items-center gap-2 rounded-sm bg-gradient-bg p-4 text-xs text-white"
              >
                <FaCircleArrowDown />
              ...
              <ul
                tabIndex={0}
                className="menu dropdown-content z-[1] mt-1.5 flex w-full flex-row rounded-sm bg-gradient-bg p-2 text-black shadow"
              >
